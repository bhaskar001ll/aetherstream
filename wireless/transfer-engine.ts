 ...
 * High-speed peer-to-peer data transport over WebRTC DataChannel and Local Wi-Fi HTTP Streaming.
 */
...
export class WirelessTransferEngine {
  ...
  private setupBroadcastChannel() {
    this.broadcastChannel.onmessage = (event) => {
      ...
      if (targetId === this.myDevice.id) {
        this.handleDirectSignal(type, signal, peer);
      }
    };
  ...
  /**
   * Periodic polling of local server signaling endpoint (/api/wireless/signal)
   */
  ...
  private createPeerConnection(isInitiator: boolean, remotePeer: PeerDevice): RTCPeerConnection {
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) {
        this.sendSignal("ICE_CANDIDATE", event.candidate, remotePeer.id);
      }
    };

    if (isInitiator) {
      const dc = pc.createDataChannel("aether-transfer", {
        ordered: true,
      });
  ...
  /**
   * Connects to a target peer using WebRTC
   */
  public async connectToPeer(targetPeer: PeerDevice): Promise<boolean> {
    ...
    await this.sendSignal("OFFER", offer, targetPeer.id);
